...
const zennRssUrl = (id: string): string => `https://zenn.dev/${id}/feed`;
...
type RawZennArticle = ZennArticle & {
  pubDate: string;
};
...
export async function fetchArticles(userId: string): Promise<ZennArticle[]> {
  ...
  return items.map(
    (entity: RawZennArticle): ZennArticle => {
      return {
        ...entity,
        pubDate: DateTime.fromHTTP(entity.pubDate),
      };
    },
  );
}
